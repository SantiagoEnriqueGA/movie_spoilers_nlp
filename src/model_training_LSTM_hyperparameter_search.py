...
from ml_utils_LSTM import SequenceDataset, ConfigurableLSTM, train, evaluate
...
def hyperparameter_search(X_train, y_train, X_test, y_test, param_grid):
    ...
    for idx, params in enumerate(param_list):
        ...
        # Avoid using dropout with a single layer
        ...
        model = ConfigurableLSTM(X_train.shape[1], hidden_dim, num_layers, dropout_rate)
        model.to(device)
...
param_grid = {
    ...
    'num_layers': [2, 3],  # Exclude 1 to avoid dropout warning
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ConfigurableLSTM(X_train.shape[1], best_params['hidden_dim'], best_params['num_layers'], best_params['dropout_rate'])
model.to(device)
