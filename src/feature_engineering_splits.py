...
joblib.dump(X_train, 'data/processed/v3/splits/base/X_train.pkl')   # Save the training data
joblib.dump(X_test, 'data/processed/v3/splits/base/X_test.pkl')     # Save the testing data
joblib.dump(y_train, 'data/processed/v3/splits/base/y_train.pkl')   # Save the training labels
joblib.dump(y_test, 'data/processed/v3/splits/base/y_test.pkl')     # Save the testing labels
joblib.dump(scaler, 'models/v3/prep/scaler.pkl')                        # Save the StandardScaler
...
joblib.dump(X_train_smote, 'data/processed/v3/splits/smote/X_train.pkl')    # Save the SMOTE-resampled training data
joblib.dump(y_train_smote, 'data/processed/v3/splits/smote/y_train.pkl')    # Save the SMOTE-resampled training labels
joblib.dump(X_test, 'data/processed/v3/splits/smote/X_test.pkl')            # Save the testing data
joblib.dump(y_test, 'data/processed/v3/splits/smote/y_test.pkl')            # Save the testing labels

# X_train_smote = joblib.load('data/processed/v3/splits/smote/X_train.pkl')   
# X_test = joblib.load('data/processed/v3/splits/smote/X_test.pkl')
# y_train_smote = joblib.load('data/processed/v3/splits/smote/y_train.pkl')
# y_test = joblib.load('data/processed/v3/splits/smote/y_test.pkl')
...
for i in range(0, X_train_smote.shape[0], 1000):            # Fit the PCA model in chunks to avoid memory issues
    ...
    initial_pca.partial_fit(X_train_smote[i:end]) # Fit the PCA model on the chunk
...
for i in range(0, X_train_smote.shape[0], 1000):                            # Fit the PCA model in chunks to avoid memory issues
    ...
    final_pca.partial_fit(X_train_smote[i:end])                   # Fit the PCA model on the chunk
...
for i in range(0, X_train_smote.shape[0], 1000):    # Transform the training data in chunks to avoid memory issues
    ...
    X_train_smote_pca = vstack([X_train_smote_pca, final_pca.transform(X_train_smote[i:end])])    # Transform and stack the chunk
...
for i in range(0, X_test.shape[0], 1000):           # Transform the testing data in chunks to avoid memory issues
    ...
    X_test_pca = vstack([X_test_pca, final_pca.transform(X_test[i:end])])   # Transform and stack the chunk


joblib.dump(X_train_smote_pca, 'data/processed/v3/splits/smote_pca/X_train.pkl')    # Save the PCA-transformed training data
joblib.dump(X_test_pca, 'data/processed/v3/splits/smote_pca/X_test.pkl')            # Save the PCA-transformed testing data
joblib.dump(y_train_smote, 'data/processed/v3/splits/smote_pca/y_train.pkl')        # Save the SMOTE-resampled training labels
joblib.dump(y_test, 'data/processed/v3/splits/smote_pca/y_test.pkl')                # Save the testing labels
joblib.dump(final_pca, 'models/v3/prep/pca.pkl')                                    # Save the PCA model