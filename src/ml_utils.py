...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, save_dir = None):
...
def train_optuna(nn_type, X_train, y_train, X_val, y_val, input_dim, device, n_trials=100, n_epochs = 25):
    ...
    study = optuna.create_study(
        direction='maximize',
        storage=f"sqlite:///optuna_study_{nn_type}.db",  # Specify the storage URL here.
        study_name=f"{nn_type}-hyperparameter-tuning",
    ...
    joblib.dump(study, f"models/v2/optuna_study_{nn_type}.pkl")
